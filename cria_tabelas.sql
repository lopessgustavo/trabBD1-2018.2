-- Usuario(__idUsuario__,nome,email,senha).
CREATE TABLE USUARIO (
	ID_USUARIO INT NOT NULL AUTO_INCREMENT,
	NOME VARCHAR(255) NOT NULL,
	EMAIL VARCHAR(255) NOT NULL,
	SENHA VARCHAR(255) NOT NULL,
	PRIMARY KEY(ID_USUARIO)
);

-- UsuarioPremium(__idUsuario__,num_cartao,cvv).
CREATE TABLE USUARIO_PREMIUM(
	ID_USUARIO INT NOT NULL,
	NUM_CARTAO INT(16) NOT NULL,
	CVV INT(3) NOT NULL,

	PRIMARY KEY(ID_USUARIO),
	
	CONSTRAINT 'USUARIO_PREMIUM_FK1'
		FOREIGN KEY(ID_USUARIO) REFERENCES USUARIO(ID_USUARIO) ON DELETE CASCADE ON UPDATE CASCADE
);

CREATE TABLE ARTISTA(
	NOME_ARTISTA VARCHAR(255) NOT NULL,
	DATA_DE_FUNDACAO DATE,
	PRIMARY KEY(NOME_ARTISTA)
);

CREATE TABLE ALBUM(
	NOME_ARTISTA VARCHAR(255) NOT NULL,
	NOME_ALBUM VARCHAR(255) NOT NULL,
	DURACAO TIME NOT NULL,
	DATA_LANCAMENTO DATE,

	PRIMARY KEY(NOME_ARTISTA,NOME_ALBUM),
	
	CONSTRAINT 'ALBUM_FK1'
		FOREIGN KEY(NOME_ARTISTA) REFERENCES ARTISTA(NOME_ARTISTA) ON DELETE CASCADE ON UPDATE CASCADE
);

CREATE TABLE MUSICA(
	NOME_ARTISTA VARCHAR(255) NOT NULL,
	NOME_ALBUM VARCHAR(255) NOT NULL,
	NOME_MUSICA VARCHAR(255) NOT NULL,
	DURACAO TIME NOT NULL,
	ORDEM INT,

	PRIMARY KEY(NOME_ARTISTA,NOME_ALBUM,NOME_MUSICA),
	CONSTRAINT "MUSICA_FK1" 
		FOREIGN KEY(NOME_ARTISTA) REFERENCES ARTISTA(NOME_ARTISTA) ON DELETE CASCADE ON UPDATE CASCADE
	CONSTRAINT "MUSICA_FK2"
		FOREIGN KEY(NOME_ALBUM) REFERENCES ALBUM(NOME_ALBUM) ON DELETE CASCADE ON UPDATE CASCADE
);

CREATE TABLE GENERO(
	NOME_GENERO VARCHAR(255) NOT NULL,

	PRIMARY KEY(NOME_GENERO)
);

CREATE TABLE PLAYLIST(
	ID_PLAYLIST INT NOT NULL AUTO_INCREMENT,
	NOME VARCHAR(255) NOT NULL,

	PRIMARY KEY(ID_PLAYLIST)
);

CREATE TABLE MUSICA_SALVA(
	ID_MUSICA_SALVA INT NOT NULL AUTO_INCREMENT,
	ID_USUARIO INT NOT NULL,
	
	PRIMARY KEY(ID_MUSICA_SALVA),
	
	CONSTRAINT 'PLAYLIST_FK1'
		FOREIGN KEY(ID_USUARIO) REFERENCES USUARIO(ID_USUARIO) ON DELETE CASCADE ON UPDATE CASCADE
);






















